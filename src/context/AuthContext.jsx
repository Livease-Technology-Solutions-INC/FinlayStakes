...
export const AuthProvider = ({ children }) => {
	...
	const goalsURL = async (
		id,
		children_education,
		capital_required_for_univerisy,
	...
	const existingProvisionsDetails = async (
		...
		const response = await fetch(
			`http://127.0.0.1:8000/app/existing_provisions_details/${encodeURIComponent(id)}/`,
			{
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
	...
	const financialPlanningShortfallURL = async (
		...
		const response = await fetch(
			`http://127.0.0.1:8000/app/financial_planning_shortfall/${encodeURIComponent(id)}/`,
			{
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
